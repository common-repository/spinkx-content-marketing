...
<span class="img-cntnr-gf-mn-dv" style="position: absolute;r;left: -5px;top: -6px;"><span class="spinkx-notify-update-bubble">'.$pending.'</span></span>
	...
	public function spinkx_cont_update_on_save_post( $post_id, $post, $update ) {
	...
	public function spinkx_cont_update_on_publish_to_publish( $post ) {
		...
		$postData = array( 'post' => $json_posts_array);
		spnxHelper::doCurl( $curl_url, $postData, false, array(), 0.01, true, false);
				...
				if (count($post_array) > 0) {
					...
					foreach ($post_array as $post) {
						...
						if ($post_status == 'publish') {
							...
							if (class_exists('Domainmap_Plugin')) {
								$current_blog_id = get_current_blog_id();
								$relative_url = str_replace(home_url(), '', get_permalink($post));
								$permalink_url =  $this->wpmu_get_mapped_domain($current_blog_id) . $relative_url;

							} else {
								$permalink_url = get_permalink($post);
							}
							$data[$post] = $permalink_url;
						}
					}
					if (count($data) > 0) {
						$settings = get_option(SPINKX_CONTENT_LICENSE);
						...
						$post_array['cuser_email'] = $spnxAdmin->getCurrentUserEmail();
						$data['key'] = $post_array;
						$json_posts_array = base64_encode(maybe_serialize($data));
						$postData = array('post' => $json_posts_array);
						$output = spnxHelper::doCurl(SPINKX_CONTENT_BAPI_URL . '/wp-json/spnx/v1/content-playlist/post/update_permalink',$postData, FALSE, array(), 0.01, true, false);
