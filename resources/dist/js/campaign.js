...
jQuery( document ).ready(function($) {
    ...
    jQuery("#wallet_amount").on("keyup blur", function(event) {

        var amount = parseInt($(this).val());
        var symbol = $(this).prev().attr('class');
        targetamount = 100;
        if(symbol.indexOf('rupee-sign') > 0) {
            symbol = 'INR';
        } else if(symbol.indexOf('dollar-sign') > 0) {
            symbol = 'USD';
            targetamount = 10;
        } else if(symbol.indexOf('euro-sign') > 0) {
            symbol = 'EURO';
            targetamount = 10;
        } else if(symbol.indexOf('pound-sign') > 0) {
            symbol = 'GBP';
            targetamount = 10;
        }
        if (amount === undefined || isNaN(amount)) {
            ...
            // document.getElementById('payment-method-button').style.backgroundColor = 'lightblue';
            if(event.type != 'keyup') {
                alert('A minimum of  '+ targetamount + ' ' + symbol + ' add in your wallet.');
            }
            $("#payment-method-button").prop("disabled", true);
...
function addMoneyForCampaign( wallet_balance ) {
    ...
    if( $budget > wallet_balance ) {
        jQuery('#wallet_amount').val(($budget - wallet_balance ).toFixed(2));
        jQuery('#add_money_wallet').trigger('click');
    } else {
