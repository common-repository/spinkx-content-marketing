	...
	function() {
		if ( ! spinkx_cont_bot_detection() && shortcode_exists( 'spinkx' ) && ! is_admin() ) {
			...
			if ( $widget_list && $spnx_widget_list_updated ) {
				...
				if ( ! $spnx_admin_manage->update_widget_list() ) {
			...
			foreach ( $widget_list as $widget ) {
				$is_mobile = (bool) $widget['is_mobile'];
				if ( $deviceType == 'desktop' && $is_mobile === false ) {
					$spnxwdArr[] = array(
						'w'         => $widget['widget_id'],
						'c'         => $license_arr['license_code'],
						's'         => $license_arr['site_id'],
						'p'         => 0,
						'is_mobile' => $is_mobile,
						'f'         => $widget['fixed'],
					);
				} elseif ( $deviceType != 'desktop' && $is_mobile === true ) {
					$spnxwdArr[] = array(
						'w'         => $widget['widget_id'],
						'c'         => $license_arr['license_code'],
						's'         => $license_arr['site_id'],
		...
		<script>
			...
			var assetsurl = '<?php echo SPINKX_CONTENT_DIST_URL; ?>';
			var spnx_cdn_url = '<?php echo SPINKX_CONTENT_DIST_URL; ?>';
			var sx_id = "";
			...
			(function (d, s) {
				...
				c.async = !0;
				c.src =  '<?php echo SPINKX_CONTENT_CDN_URL; ?>js/spnx_init.min.js';
					...
					if (event.target.nodeName === "SCRIPT") {
						if (event.target.getAttribute("src").indexOf('embeds.js') != -1) {
							if (undefined !== window.spnx) {
								...
								b.async = !0;
								b.src = '<?php echo SPINKX_CONTENT_CDN_URL; ?>js/spinkxut.js';
...
add_action( 'transition_post_status', array( new spnxAdminManage(), 'spinkx_cont_update_status_transitions' ), 10, 3 );
add_action( 'save_post', array( new spnxAdminManage(), 'spinkx_cont_update_on_save_post' ), 10, 3 );
